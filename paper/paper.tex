...
\caption{Subgraph isomorphism. Shaded code is to be ignored for the non-restarting algorithm.}
\label{algorithm:sip}
\end{algorithm}
...
smallest domain first, tiebreaking on ?? degree. Variable ordering is not the focus of this
...
?? In \cref{figure:others} we show that our solver is the bestest.
...
?? Stick in Glasgow2, Glasgow3, VF2, SND if we can get it to run on the larger set of instances,
