...
\begin{abstract}
    ...
    effective in offsetting mistakes made by value-ordering heuristics in this situation.  We then
...
and with edges between vertices $v$ and $w$ if there are at least $n$ simple paths of length exactly
...
current state of the art \citep{DBLP:conf/ijcai/McCreeshPT16}.
...
\begin{figure}[h]
    \centering
    \includegraphics*{gen-graph-scatter-constant.pdf}

    \caption{Constant restarts?}
    \label{figure:scatter-constant}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Parallel Luby}
    \label{figure:parallel-luby}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics*{gen-graph-scatter-parallel-constant.pdf}

    \caption{Parallel Constant}
    \label{figure:parallel-constant}
\end{figure}
