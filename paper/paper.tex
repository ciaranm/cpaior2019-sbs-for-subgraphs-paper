...
$\mathcal{P}$ are mapped to adjacent vertices in $\mathcal{T}$ (including that vertices with loops
in $\mathcal{P}$ may only be mapped to vertices with loops in $\mathcal{T}$). We write $v \mapsto
v'$ to mean that pattern vertex $v$ is mapped to target $v'$ under such a mapping. Finally, we
define a function $\FuncSty{compatible}(v, w)$ to mean that mapping $v \mapsto w$ is feasible based
upon both the loop rule, and the neighbourhood degree sequence relation of
...
\Cref{figure:scatter-heuristics} gives a detailed look at these results. This scatter plot
