...
\authorrunning{C. McCreesh, P. Prosser and J. Trimble}

\Copyright{Ciaran McCreesh, Patrick Prosser and James Trimble}
...
\keywords{Subgraph isomorphism, combinatorial search, rich search}

\funding{This work was supported by the Engineering and Physical Sciences Research Council [grant
numbers EP/026842/1 and EP/M508056/1]}

\acknowledgements{The authors would like to thank Ruth Hoffmann, Christophe Lecoutre, Christine
...
\nl \Begin{
    ...
    \nl \KwSty{global} $L \gets \big[ (\mathcal{P},\ \mathcal{T}), \siplabel{supplemental}
        ...
        (\mathcal{P}^{2,2},\ \mathcal{T}^{2,2}), (\mathcal{P}^{3,2},\ \mathcal{T}^{3,2}),
        (\mathcal{P}^{4,2},\ \mathcal{T}^{4,2}) \big]$ \;
    \nl \ForEach{\siplabel{initstart}$v \in \vertexset(\mathcal{P})$}{
        \nl $D_v \gets \vertexset(\mathcal{T})$\;
        \nl \lForEach{$(P,\,T) \in L$}{$D_v \gets \{ w \in D_v : $ \siplabel{degreefiltering} $v \sim_P v \Rightarrow w \sim_T w~\wedge$
...
\nl \Begin{
    \nl \While{$D_v \gets$~\textnormal{a unit domain from}~$D$}{
        ...
        \nl \ForEach{$D_w \in D - D_v$}{
            ...
            \nl \lForEach{$(P,\,T) \in L~\KwSty{where}~v \sim_P w$}{$D_w \gets D_w \cap \neighbourhood_T(v')$}
...
For the variable-ordering heuristic on \siplineref{variableordering} of \algorithmref{algorithm:sip}, we use
smallest domain first, tiebreaking on highest degree. Variable ordering is not the focus of this
paper---as far as possible, we use this same dynamic ordering throughout the paper, so as not to
affect the performance of the algorithm on unsatisfiable instances.  For value ordering, which
occurs on \siplineref{valueordering} of \algorithmref{algorithm:sip}, we will initially select vertices from
highest degree to lowest degree; we return to value ordering in the second half of this paper. Note
that the variable-ordering heuristic affects both unsatisfiable and satisfiable instances, whilst
the value-ordering heuristic is relevant only for satisfiable instances.
...
\begin{figure}[p]
    ...
    with restarts, but without nogood recording.}
    \label{figure:scatter-random-goods}
\end{figure}
...
\begin{figure}[p]
    ...
    \caption{Comparing basic backtracking with the degree heuristic, versus the random heuristic
    with restarts and nogood recording.}
...
\begin{figure}[p]
    ...
    \caption{Comparing McSplit${\downarrow}$ with and without restarts and nogood recording on an
    instance by instance basis.}\label{figure:mcsplit}
\end{figure}
